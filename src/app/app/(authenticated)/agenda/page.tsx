...
function ListCard({ event, onClick }: { event: AgendaEvent; onClick: () => void }) {
    ...
    return (
        <button
            className="w-full text-left flex items-center gap-4 p-4 bg-white rounded-2xl border border-slate-100 hover:border-slate-200 hover:shadow-md transition-all duration-200 group"
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-0.5 text-xs text-slate-500">
                    ...
                    {event.studentName && !event.instructor && (
                        <span className="flex items-center gap-1 truncate">
                            <User className="h-3 w-3" />{event.studentName}
                        </span>
                    )}
                    {event.room && (
                        <span className="flex items-center gap-1 truncate">
            ...
            <div className="flex items-center gap-2 shrink-0">
                {typeof event.capacity === 'number' && (
                    <span className="flex items-center gap-1 text-xs font-semibold text-slate-500 bg-slate-50 border border-slate-200 px-2 py-1 rounded-full">
                ...
                <div className="flex flex-col flex-1 bg-white border border-slate-100 rounded-3xl overflow-hidden min-h-0 shadow-sm">
                    ...
                    <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 shrink-0 bg-white">
                        ...
                        <div className="flex items-center gap-1.5">
                            <button
                                onClick={handlePrev}
                                className="h-8 w-8 flex items-center justify-center rounded-full hover:bg-slate-100 transition-colors text-slate-600"
                            >
                                <ChevronLeft className="h-4 w-4" />
                            </button>
                            <button
                                onClick={() => setCurrentDate(new Date())}
                                className="h-8 px-3 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-full transition-colors"
                            >
                                Hoje
                            </button>
                            <button
                            ...
                            <h2 className="text-base font-bold text-slate-800 ml-1">{headerTitle}</h2>
                            {totalEventsToday > 0 && (
                                <span className="ml-1 text-[10px] font-bold bg-bee-amber/20 text-amber-700 px-2 py-0.5 rounded-full">
                        ...
                        <div className="flex items-center bg-slate-100 rounded-full p-0.5 gap-0.5">
                            {(['day', 'week', 'month'] as const).map(v => (
                                <button
                                    ...
                                    onClick={() => setView(v)}
                                    className={cn(
                                        'px-3.5 py-1.5 text-[11px] font-bold tracking-wide rounded-full transition-all',
                                        view === v
